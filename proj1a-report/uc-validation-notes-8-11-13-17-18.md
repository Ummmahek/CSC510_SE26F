...
Expected: **29 tests — 28 pass, 1 intentional failure** (the `[DOC EXPECTATION]`
...
- Firestore is mocked with PR #1's `fakeFirestore.js` for UC8/13/18. UC11 needs
  ...
  which is mocked at the axios entry file `server/node_modules` actually
  resolves (two resolution traps documented in the test header; a naive
  `jest.mock('axios')` silently mocks nothing and the suite makes REAL calls
  to Google).
...
| UC17 source inspection (5 tests) | Client runner broken; assert the fabricated-courier finding on source | Map is customer-facing; interpolated marker; hardcoded 20s; "Start Delivery" button; no real position source | PASS (all five findings confirmed in source) |
...
| UC18 two-ledgers finding | totalEarnings written but never served | A diverged ledger value (999) is invisible to GET /orders | PASS (finding confirmed) |

## Traceability additions

| Test file | Use case | What it proves |
|---|---|---|
| `uc8-rate-order.test.js` (8) | UC8 | All five documented guards hold; rating is immutable once set; no stored average exists (derived at read) |
| `uc11-voice-control.test.js` (7) | UC11 | All guards hold with Gemini mocked; upstream errors pass through; the headline gap (no ordering action) is pinned by an intentional red test |
...
| `uc17-delivery-map.test.js` (5) | UC17 | The customer-facing "tracking" map is a hardcoded 20s simulation with no real position source (source-inspection; client runner broken as inherited) |
